...
(function() {
    ...
    function addMobileSafariSupport() {
        // Touch/click handling is attached explicitly to created buttons; no global overrides.
        // Do not modify page viewport to preserve accessibility and avoid iOS quirks.
        ...
        mobileStyle.textContent = `
            ...
            /* Landscape phones: 3 columns, avoid wrapping */
            ...
            .sff-clean-panel input[type="text"],
            .sff-clean-panel input[type="number"],
            .sff-clean-panel textarea {
                -webkit-appearance: none !important;
                appearance: none !important;
                border-radius: 6px !important;
            }
            ...
            .sff-panel-content::-webkit-scrollbar {
                width: 6px !important;
            }
            
            .sff-panel-content::-webkit-scrollbar-track {
                background: #f1f1f1 !important;
                border-radius: 3px !important;
            }
            
            .sff-panel-content::-webkit-scrollbar-thumb {
                background: #fc5200 !important;
                border-radius: 3px !important;
            }
            
            .sff-panel-content::-webkit-scrollbar-thumb:hover {
                background: #e04a00 !important;
            }
            
            /* Ensure checkboxes are always visible */
            .sff-chip input[type="checkbox"] {
    ...
    // Initialize mobile Safari support
    addMobileSafariSupport();
    ...
    const UIModule = {
        ...
        setupEvents(btn, panel, secondaryFilterBtn, secondaryKudosBtn) {
            ...
            if (secondaryFilterBtn) {
                ...
                secondaryFilterBtn.addEventListener('touchstart', (e) => {
                    console.log('🔥 Secondary filter button touched!');
                    e.preventDefault();
                    e.stopPropagation();
                    togglePanel();
                }, { passive: false });
    ...
    const LogicModule = {
        ...
        isClubPost(node) {
            ...
            try {
                // 1) Explicit club link in header like: <div class="clubMemberPostHeaderLinks"> ... <a class="clubLink" href="/clubs/..."> ...
                if (el.querySelector?.('.clubMemberPostHeaderLinks a.clubLink[href^="/clubs/"]')) return true;

                // 2) Club avatar present: <a data-testid="club-avatar" href="/clubs/...">
                if (el.querySelector?.('a[data-testid="club-avatar"][href^="/clubs/"]')) return true;

                // 3) Post details URL pointing to a club post: <a data-testid="post-details-url" href="/clubs/.../posts/...">
                ...
                // 4) Any primary container link to /clubs/ inside the entry
            ...
            try {
                ...
                entries.forEach(entry => {
                    ...
                    const hasJoinCta = !![...(entry.querySelectorAll('button, a[role="button"], a'))]
                        .some(el => /\bjoin club\b/i.test(el.textContent || ''));
                    if (hasJoinCta) {
                        const container = entry.closest('[id^="feed-entry-"]') || entry;
                        toHide.add(container);
                    }
